...
import * as customShapes from '../shapes/custom/title';
import * as svgShapes from '../shapes/custom/polygon';
import * as chartShapes from '../shapes/custom/chart_knob';
...
export class StencilService {
    ...
    create(paperScroller: ui.PaperScroller, snaplines: ui.Snaplines) {
        this.stencil = new ui.Stencil({
            ...
            paperOptions: function() {
                return {
                    model: new dia.Graph({}, {
                        cellNamespace: myShapes
                    }),
                    cellViewNamespace: myShapes
                };
            },
            search: {
                '*': ['type', 'name']
        ...
        this.stencil.render();
        this.stencil.el.style.width = '350px'; 
        ...
        this.tooltipGraph = new dia.Graph({}, { cellNamespace: myShapes });
    ...
    buildTooltipContent(cell: dia.Cell) {
        ...
        //в стоковой версии tooltip-ы работаею. Что то не так с классом
    setTimeout(() => {
        const tooltip = document.querySelector('.joint-tooltip')
        if(tooltip){
            tooltip.innerHTML = null
            tooltip?.appendChild(documentFragment)
            return tooltip;
        }
    }, 100)
